...
//            self.createSquares()
    ...
    func createSquares(name: String) {
        ...
        let square = SKSpriteNode(texture: nil, color: .purple, size: size)
        square.name = name
        ...
        let action = SKAction.customAction(withDuration: 2.0, actionBlock: { (square, _) in
            let square = square as! SKSpriteNode
            square.position.y += 4
            ...
            }else if (square.position.y >= self.view!.bounds.height/2) {
                square.color = UIColor.orange
            }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if name != nil {
                ...
                let nodeToRemove = self.childNode(withName: name!)
